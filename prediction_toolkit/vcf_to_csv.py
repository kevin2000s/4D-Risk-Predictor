...
"""Convert VCF files (Snippy or generic) to the long-format CSV used by the predictor."""
import argparse
import glob
import gzip
import os
...
def _load_reference_snp_ids(model_dir=None):
    """Load the reference SNP ID list (CHROM_POS) used during training."""
...
def _open_vcf(vcf_path):
    """Open a VCF file, either plain or gzip-compressed."""
...
def _parse_gt(gt_val):
    """Return True if the genotype contains a non-reference allele."""
    if gt_val in (".", "./.", ".|."):
        return False
...
def _parse_vcf_samples(vcf_path, ref_snp_ids):
    """Parse a VCF and return SNP records with the samples carrying the ALT allele."""
    snp_data = {}
...
def _collect_snippy_vcf_files(snippy_dir):
    """Collect per-sample snps.vcf files from a Snippy output directory."""
    vcf_files = []
...
def _build_long_format(snp_data):
    """Convert SNP records to the long-format DataFrame."""
    rows = []
...
def convert_multisample_vcf(vcf_path, ref_snp_ids):
    """Convert a single multi-sample VCF."""
...
def convert_vcf_list(vcf_files, ref_snp_ids):
    """Convert several single-sample VCF files."""
...
def convert_snippy_folder(snippy_dir, ref_snp_ids):
    """Convert a Snippy output folder."""
